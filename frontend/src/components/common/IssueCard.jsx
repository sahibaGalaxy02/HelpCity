...
export default function IssueCard({ issue, showDelete = false }) {
  ...
  return (
    <Link
      to={`/issues/${issue._id}`}
      className="card group block overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-card-hover animate-fade-in"
    >
      {/* Image */}
      <div className="relative aspect-video overflow-hidden bg-slate-100">
        {issue.imageUrl ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-sky-100 via-white to-orange-50 text-gray-400">
            <span style={{ transform: 'scale(2.5)' }}>
              {getCategoryIcon(issue.category, 20)}
            </span>
          </div>
        )}
        ...
        <div className="absolute top-3 left-3">
          <span className={`category-badge flex items-center gap-1 ${getCategoryStyle(issue.category)}`}>
            {getCategoryIcon(issue.category, 11)}
            {issue.category}
          </span>
        </div>
      </div>
